...
| `GLINT_FLAT_BG` | `env` or shell | unset | Set to `1` to draw the pill on a single solid background instead of the gradient. Use it on terminals whose own GPU renderer manages color its own way (Display P3 / non-sRGB blending) and smears the subtle glass gradient into blotchy tones. Auto-enabled under Zentty (detected by the `ZENTTY_*` env vars it injects), so you don't need to set it there |
...
**Solid-background fallback.** The gradient leans on dozens of subtly different truecolor backgrounds per row. A terminal that manages color its own way, reading the values as Display P3 or blending in the wrong space, smears those close tones into blotchy, off-hue patches, worst across the wide, low-saturation pill body. Setting `GLINT_FLAT_BG=1` drops the gradient for one solid background (the glass center color, caps included), which renders identically anywhere. Zentty triggers this automatically: it injects `ZENTTY_*` variables into the environment, a signal that survives both Claude Code's env sanitizing and the session running under a detached daemon, where neither the terminal's identity (`__CFBundleIdentifier`, `TERM_PROGRAM`) nor the process tree would reveal it.
