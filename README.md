...
shows:    model · effort · thinking lamp · project · branch + dirty count · context bar and %
          · which account · quota pace · 5h and 7d windows · version · API status · network · clock
...
- 🕒 **Clock**: today's date and the time, `15/09 16:36`, at the end of the bar. Claude Code only tells the model the date once per session and the model sometimes drifts; a clock that redraws with the bar is the cheapest fix, and every reset time on the same line ("↻3d 1h") reads against it. The date can be switched off in the panel, leaving `16:36`.
...
| **Clock** | `15/09 16:36` | Current date and time, redrawn with the bar. Date optional |
